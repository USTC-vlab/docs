...
若因实验需要将虚拟机内的网络端口开放到公网访问，我们可以在网关上配置 NAT 端口映射，此时虚拟机内能直接得到外部访问的 IP 和端口。由于合规性要求，我们会严格处理端口映射的申请，并且不会映射低端口（0-1023）和一些敏感端口（如 6379、8080 等）。端口映射仅支持 TCP 和 UDP 协议，暂不支持 IPv6。
