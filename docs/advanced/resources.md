...
目前创建的虚拟机配置统一为 2 核心 CPU、6 GB 内存与 16 GB 存储空间，该配置足够进行大部分计算机学院课程学习活动（如使用 Vivado 等）。对于 LXC 虚拟机，[Vlab 实验软件](images.md#vlab-software)不占用虚拟机的存储空间。
...
!!! info "硬盘容量"
    ...
    由于存储不是动态分配的（而是全额预留的容量），且调整较为复杂，因此如果你需要超过 64 GB 的总容量，我们建议你先看看[科大超算中心 GitLab](https://git.ustc.edu.cn/) 和[科大云盘](https://pan.ustc.edu.cn/)是否能够满足你的存储需求，再考虑向我们申请额外的存储。
